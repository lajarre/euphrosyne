...
def presigned_raw_data_list_url_view(request, project_id: int):
    url = create_presigned_raw_data_list_url(project_id)
    data = {"url": url}
...
@project_membership_required
@require_http_methods(["POST"])
def presigned_raw_data_upload_url_view(request, project_id: int):
    url = create_presigned_raw_data_upload_post(project_id)
    data = {"url": url}
    return JsonResponse(data)
...
def presigned_raw_data_download_url_view(request, project_id: int):
    ...
    if not key.startswith(f"projects/{project_id}/raw_data/"):
        return JsonResponse(
